...
import { GoldilocksGovernor } from "../src/governance/GoldilocksGovernor.sol";
import { GoldilocksTimelockController } from "../src/governance/GoldilocksTimelockController.sol";
import { GovernanceLocks } from "../src/governance/GovernanceLocks.sol";
...
contract TestBase is Test {
  ...
  Goldiswap goldiswap;
  Goldilend goldilend;
  GovernanceLocks govlocks;
  GoldilocksTimelockController timelock;
  Goldilocked goldilocked;
  GoldilocksGovernor goldigov;
  Honey honey;
  Bera bera;
  HoneyComb honeycomb;
  Beradrome beradrome;
  ...
  function setUp() public {
    ...
    govlocks = new GovernanceLocks(address(goldiswap), address(goldilockedComputed));
    address[] memory proposers = new address[](0);
    address[] memory executors = new address[](0);
    timelock = new GoldilocksTimelockController(1, proposers, executors, address(this));
    goldigov = new GoldilocksGovernor(govlocks, timelock);
    timelock.grantRole(keccak256("PROPOSER_ROLE"), address(goldigov));
    timelock.grantRole(keccak256("EXECUTOR_ROLE"), address(goldigov));
    timelock.grantRole(keccak256("CANCELLER_ROLE"), address(goldigov));
