...
contract RebaseGoldilendHandler is BaseHandler {
  ...
  function borrow(
    ...
    try RebaseGoldilend(address(rebasegoldilend)).borrow(borrowAmount, duration, address(bandbear), actualTokenId) {
  ...
  function renew(
    ...
    try RebaseGoldilend(address(rebasegoldilend)).renew(loanId, newDuration, newBorrowAmount) {
